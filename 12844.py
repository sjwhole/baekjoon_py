...
class Solve:
    def __init__(self):
        ...
        idx = int(pow(2, int(log2(self.N)) + 2))
        self.tree = [0] * idx
        self.lazy = [0] * idx
    ...
    def update_lazy(self, node, start, end):
        if self.lazy[node]:
            self.tree[node] ^= ((end - start + 1) % 2) * self.lazy[node]
            if start != end:
    ...
    def update_range(self, node, start, end, left, right, diff):
        ...
        if left <= start and end <= right:
            self.tree[node] ^= ((end - start + 1) % 2) * diff
            if start != end:
    ...
    def sol(self):
        for _ in range(self.M):
            ...
            if temp_list[0] == 1:
                self.update_range(1, 0, self.N - 1, temp_list[1], temp_list[2], temp_list[3])
            else:
                print(self.add(1, 0, self.N - 1, temp_list[1], temp_list[2]))
